...
Serial.println(robotAngle);
    ...
    powerFL = sin(toRadians(robotAngle - 60));
    powerRL = sin(toRadians(robotAngle - 120));
    powerRR = sin(toRadians(robotAngle - 240));
    powerFR = sin(toRadians(robotAngle - 300));
...
maxval = max(max(abs(powerFR), abs(powerFL)), max(abs(powerRR), abs(powerFL)));
    ...
    analogWrite(speedRR, (powerRR*1.5));
...
direction = power < 0 ? LOW : HIGH;
power = 0.35 *(255 * (abs(power) / maxValue));
